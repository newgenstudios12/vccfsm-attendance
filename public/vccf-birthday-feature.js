(()=>{
'use strict';
if(window.__VCCF_BIRTHDAY_FEATURE_V1__)return;
window.__VCCF_BIRTHDAY_FEATURE_V1__=true;
...
function stripBirthdayRequired(){
  ...
  if(label)label.textContent='Birthday (Optional)';
...
function birthdayItems(){
  ...
  return members
    .filter(m=>m?.birthday && /^\d{4}-\d{2}-\d{2}$/.test(String(m.birthday)) && Number(String(m.birthday).slice(5,7))===month+1)
    .map(m=>({
      ...m,
      day:Number(String(m.birthday).slice(8,10)),
      isToday:Number(String(m.birthday).slice(8,10))===today
    }))
...
function boot(){
  ...
  renderBirthdayMonth();
  const mo=new MutationObserver(()=>{stripBirthdayRequired();renderBirthdayMonth();});
  mo.observe(document.body,{subtree:true,childList:true,attributes:true,attributeFilter:['class','required']});
  document.addEventListener('click',()=>setTimeout(stripBirthdayRequired,0),true);
  ...
  if(typeof originalRefresh==='function'&&!window.__VCCF_BIRTHDAY_REFRESH_PATCH__){
    ...
    window.refresh=function(){
      ...
      setTimeout(()=>{stripBirthdayRequired();renderBirthdayMonth();},50);
      return result;
    };
  }
}
