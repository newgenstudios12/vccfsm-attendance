...
@media(max-width:980px){.bssg-member-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}@media(max-width:680px){.bssg-photo-actions{display:grid;grid-template-columns:1fr}.bssg-preview-grid{grid-template-columns:1fr}.bssg-preview-item,.bssg-preview-grid.one .bssg-preview-item{aspect-ratio:16/9;max-height:none}.bssg-member-grid{grid-template-columns:1fr}.bssg-member-head{align-items:flex-start;flex-direction:column}.bssg-lightbox{padding:10px}.bssg-lightbox-nav{width:40px;height:52px}.bssg-lightbox-stage img{max-height:78vh}}
...
function decoratePreview(){
 ...
 const box=document.createElement('div');box.className='bssg-preview-gallery';box.innerHTML='<div class="bssg-preview-grid '+(list.length===1?'one':'')+'">'+list.map((photo,index)=>'<div class="bssg-preview-item"><img src="'+esc(photo.image_url)+'" alt="Bible Study summary picture '+(index+1)+'"></div>').join('')+'</div><span class="bssg-preview-caption">'+list.length+' picture'+(list.length===1?'':'s')+' attached to this Bible Study submission · Click a picture to enlarge</span>';body.prepend(box);bindImages(box,list,'Bible Study Summary')
}
