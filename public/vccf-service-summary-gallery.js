...
const canDeleteSummary=()=>role()==='admin';
...
async function deleteSummary(row,button){
 ...
 try{
  ...
  closePreview();
  window.dispatchEvent(new CustomEvent('vccf-service-summary-deleted',{detail:{summaryId:String(row.id)}}));
  await reconcile();
  try{await window.VCCFServiceAttendance?.refresh?.()}catch(error){console.warn('Bible Study attendance refresh',error)}
 }catch(error){
  if(button&&document.body.contains(button)){button.disabled=false;button.textContent=old||'Delete Summary'}
  alert(error?.message||'Unable to delete this Bible Study summary.');
 }
}
