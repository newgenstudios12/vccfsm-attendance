(()=>{
'use strict';
if(window.__VCCF_SERVICE_SUMMARY_GALLERY__)return;
window.__VCCF_SERVICE_SUMMARY_GALLERY__=true;
...
let scheduled=0,previewRow=null,escHandler=null;

function styles(){
  if(document.getElementById('vccfServiceSummaryGalleryStyles'))return;
  const s=document.createElement('style');s.id='vccfServiceSummaryGalleryStyles';
  s.textContent=`
...
function currentType(){return document.getElementById('serviceAttendanceType')?.value||''}
function currentHost(){return document.getElementById('serviceSummaryHost')}
function removeLegacyRecent(host){
  [...host.querySelectorAll(':scope > .service-summary-card')].forEach(card=>{if(card.querySelector('h3')?.textContent?.trim()==='Recent summaries')card.remove()});
}
...
function bindFilter(shell,rows){
  const input=shell.querySelector('[data-summary-search]'),select=shell.querySelector('[data-summary-status]'),grid=shell.querySelector('.service-summary-gallery-grid');
  const render=()=>{
    const q=String(input?.value||'').trim().toLowerCase(),status=String(select?.value||'');
    const filtered=rows.filter(row=>{
      const hay=[row.title,fmtDate(row.summary_date),areaName(row.area_id),row.barangay,row.notes,statusLabel(row.workflow_status)].join(' ').toLowerCase();
      return (!q||hay.includes(q))&&(!status||String(row.workflow_status||'draft')===status);
    });
    grid.innerHTML=filtered.length?filtered.map(card).join(''):'<div class="service-summary-gallery-empty">No Bible Study summaries match these filters.</div>';
    grid.querySelectorAll('[data-preview-service-summary]').forEach(b=>b.onclick=()=>{const row=rows.find(x=>String(x.id)===b.dataset.previewServiceSummary);if(row)openPreview(row)});
  };
...
async function renderGallery(){
  ...
  if(!host||currentType()!=='bible_study'||host.querySelector('[data-service-summary-gallery]'))return;
  removeLegacyRecent(host);
  const shell=document.createElement('section');shell.className='service-summary-gallery-shell card';shell.dataset.serviceSummaryGallery='1';
  shell.innerHTML='<div class="service-summary-gallery-head"><div><h3>Bible Study Summary Gallery</h3><p>Browse previous Bible Study attendance summaries and preview the full record before reopening it.</p></div><div class="service-summary-gallery-tools"><input type="search" data-summary-search placeholder="Search date, area, barangay…"><select data-summary-status><option value="">All statuses</option><option value="draft">Draft</option><option value="submitted">Submitted</option><option value="approved">Approved</option></select></div></div><div class="service-summary-gallery-grid"><div class="service-summary-gallery-empty">Loading Bible Study summaries…</div></div>';
  ...
  try{
    ...
    if(!shell.isConnected||currentType()!=='bible_study')return;
    bindFilter(shell,res.data||[]);
  }catch(error){
    if(shell.isConnected)shell.querySelector('.service-summary-gallery-grid').innerHTML='<div class="service-summary-gallery-empty">'+esc(error.message||'Unable to load Bible Study summaries.')+'</div>';
  }
}

function closePreview(){
  document.getElementById('serviceSummaryPreviewOverlay')?.remove();previewRow=null;
...
function openInAttendance(row){
  ...
  typeEl.value='bible_study';area.value=row.area_id||'';barangay.value=row.barangay||'';date.value=row.summary_date||date.value;
  area.dispatchEvent(new Event('change',{bubbles:true}));
  setTimeout(()=>{barangay.value=row.barangay||'';date.value=row.summary_date||date.value;date.dispatchEvent(new Event('change',{bubbles:true}));setTimeout(()=>document.querySelector('#serviceSummaryHost > .service-summary-card')?.scrollIntoView({behavior:'smooth',block:'start'}),180)},80);
}
function openPreview(row){
  closePreview();previewRow=row;styles();
  ...
  wrap.querySelector('.service-summary-preview-x').onclick=closePreview;wrap.querySelector('[data-close-service-preview]').onclick=closePreview;wrap.querySelector('[data-open-service-summary]').onclick=()=>openInAttendance(row);wrap.onclick=e=>{if(e.target===wrap)closePreview()};
  escHandler=e=>{if(e.key==='Escape')closePreview()};document.addEventListener('keydown',escHandler);
}

function queue(){clearTimeout(scheduled);scheduled=setTimeout(()=>renderGallery(),90)}
