...
function navBadge(){
  document.querySelectorAll('.followup-nav-badge').forEach(x=>x.remove());
  const b=document.querySelector('[data-route="members"]');if(!b||!allowed()||!rows.length)return;const badge=document.createElement('span');badge.className='followup-nav-badge';badge.textContent=rows.length>99?'99+':String(rows.length);badge.title=rows.length+' member follow-up alert'+(rows.length===1?'':'s');b.appendChild(badge)
}
...
async function refresh(force=false){
  if(!allowed()){rows=[];renderMembers();navBadge();profileAlert();return}
  if(loading)return;if(!force&&Date.now()-lastLoaded<5*60*1000){renderMembers();navBadge();profileAlert();return}
  const client=db();if(!client)return;loading=true;renderMembers();
  try{const result=await client.rpc('get_member_followup_alerts',{p_days:THRESHOLD_DAYS});if(result.error)throw result.error;rows=result.data||[];lastLoaded=Date.now();window.VCCFFollowupAlerts={thresholdDays:THRESHOLD_DAYS,rows,refresh:()=>refresh(true)}}catch(error){console.warn('Follow-up alerts',error);rows=[]}finally{loading=false;renderMembers();navBadge();profileAlert()}
}
function queue(){clearTimeout(timer);timer=setTimeout(()=>{renderMembers();navBadge();profileAlert();void refresh(false)},100)}
function watch(){if(observer)return;observer=new MutationObserver(()=>queue());observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class']})}
