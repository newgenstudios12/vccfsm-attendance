...
function renderEvents(){
  ...
  const rows=data.events.map(e=>{
    ...
    return '<tr><td><b>'+esc(e.title)+'</b><div class="cms-sub">'+esc(e.event_type)+' · '+esc(scope)+'</div></td><td>'+fmtDateTime(e.start_at)+'</td><td>'+esc(e.location||'—')+'</td><td>'+participation+'</td><td>'+badge(e.status,e.status==='Completed'?'ok':'')+'</td><td class="cms-actions">'+(can?'<button class="cms-small" data-event-attendance="'+e.id+'">Attendance</button><button class="cms-small" data-event-edit="'+e.id+'">Edit</button>'+(mode==='attendance_only'?'':'<button class="cms-small" data-event-register="'+e.id+'">Register</button>'):'')+'</td></tr>';
  }).join('');
  ...
  content().querySelectorAll('[data-event-attendance]').forEach(b=>b.onclick=()=>renderEventAttendance(b.dataset.eventAttendance));
