...
let data={albums:[],photos:[],summaries:[],summaryPhotos:[]};

function allAlbums(){
  const manual=(data.albums||[]).map(a=>({...a,auto:false,photos:(data.photos||[]).filter(p=>p.album_id===a.id).sort((x,y)=>(x.sort_order||0)-(y.sort_order||0))}));
  const sunday=(data.summaries||[]).map(s=>({...s,id:'summary:'+s.id,summary_id:s.id,auto:true,title:'Sunday Worship · '+labelDate(s.summary_date),album_date:s.summary_date,description:s.notes||'',photos:(data.summaryPhotos||[]).filter(p=>p.summary_id===s.id).sort((x,y)=>(x.sort_order||0)-(y.sort_order||0))}));
  return [...manual,...sunday].sort((a,b)=>String(b.album_date||b.created_at||'').localeCompare(String(a.album_date||a.created_at||'')));
}
async function load(){
  const [albums,photos,summaries,summaryPhotos]=await Promise.all([
    ...
    sb().from('cms_sunday_event_summaries').select('id,title,summary_date,notes,posted_at').eq('summary_type','sunday').eq('workflow_status','posted').order('summary_date',{ascending:false}),
    sb().from('cms_summary_photos').select('id,summary_id,image_url,caption,sort_order,created_at').order('sort_order')
  ]);
  const err=albums.error||photos.error||summaries.error||summaryPhotos.error;if(err)throw err;
  data={albums:albums.data||[],photos:photos.data||[],summaries:summaries.data||[],summaryPhotos:summaryPhotos.data||[]};
}
function cover(album){return album.photos?.[0]?.image_url||''}
function render(){
  const el=document.getElementById('gallery');if(!el)return;
  const albums=allAlbums();
  el.innerHTML='<section class="gallery-hero card"><div><span class="gallery-kicker">VCCF SANTA MARIA</span><h2>Gallery</h2><p>Church albums and posted Sunday highlights. Profile and member avatar pictures are kept separate and are never included here.</p></div>'+(isAdmin()?'<button id="newGalleryAlbum" class="btn" type="button">+ New album</button>':'')+'</section><div class="gallery-section-head"><div><b>'+albums.length+' album'+(albums.length===1?'':'s')+'</b><span>Every posted Sunday Summary appears automatically as an album.</span></div></div><div class="gallery-album-grid">'+(albums.length?albums.map(albumCard).join(''):'<div class="gallery-empty card"><b>No albums yet</b><span>Admins can create church albums. Posted Sunday Summaries will also appear here automatically.</span></div>')+'</div>';
...
function albumCard(a){
  const image=cover(a),count=a.photos?.length||0;
  return '<button class="gallery-album-card card" type="button" data-gallery-album="'+esc(a.id)+'"><div class="gallery-album-cover">'+(image?'<img src="'+esc(image)+'" alt="'+esc(a.title)+'" loading="lazy">':'<div class="gallery-album-placeholder"><span>VCCF</span></div>')+(a.auto?'<span class="gallery-auto-badge">Sunday Summary</span>':'')+'</div><div class="gallery-album-copy"><span>'+esc(labelDate(a.album_date))+'</span><h3>'+esc(a.title)+'</h3><p>'+count+' photo'+(count===1?'':'s')+'</p></div></button>';
}
function openAlbum(id){
  const album=allAlbums().find(a=>String(a.id)===String(id));if(!album){render();return}
  const el=document.getElementById('gallery');
  el.innerHTML='<div class="gallery-detail-head"><button id="backGallery" class="back-button" type="button">← Back to gallery</button>'+(album.auto?'<span class="gallery-auto-badge static">Automatic Sunday album</span>':(isAdmin()?'<div class="gallery-admin-actions"><button id="editGalleryAlbum" class="btn secondary" type="button">Edit album</button><button id="deleteGalleryAlbum" class="btn secondary danger-text" type="button">Delete album</button></div>':''))+'</div><section class="gallery-album-detail card"><div><span class="gallery-kicker">'+(album.auto?'POSTED SUNDAY SUMMARY':'CHURCH ALBUM')+'</span><h2>'+esc(album.title)+'</h2><p>'+esc(album.description||'')+'</p><small>'+esc(labelDate(album.album_date))+' · '+(album.photos?.length||0)+' photo'+((album.photos?.length||0)===1?'':'s')+'</small></div>'+(!album.auto&&isAdmin()?'<label class="gallery-upload-btn">+ Add photos<input id="galleryAlbumUpload" type="file" accept="image/jpeg,image/png,image/webp" multiple></label>':'')+'</section><div id="galleryPhotoGrid" class="gallery-photo-grid">'+photoGrid(album)+'</div><div id="galleryMessage" class="gallery-message"></div>';
...
function photoGrid(album){
  if(!album.photos?.length)return '<div class="gallery-empty card"><b>No photos in this album yet</b><span>'+(album.auto?'Photos attached to the Sunday Summary will appear here automatically.':'Use Add photos to upload church photos.')+'</span></div>';
