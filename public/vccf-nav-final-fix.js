...
function activate(v,btn=null){
 ...
 const id=coreIds[v]||v;const target=document.getElementById(id);
 if(!target)return false;
 qa('.main .view').forEach(x=>x.classList.remove('active'));target.classList.add('active');
 markActive(btn);setTitle(v);closeMobile();
...
function showChurchManagement(btn=null){
 ...
 if(direct){qa('.main .view').forEach(x=>x.classList.remove('active'));direct.classList.add('active');markActive(btn||q('[data-suite-nav]'));setTitle('church-management');closeMobile();return true}
 const suiteBtn=q('[data-suite-nav]');
 if(suiteBtn&&suiteBtn!==btn){try{suiteBtn.click();return true}catch{}}
 return false;
}
function ensureChurchLoader(){
 if(q(`script[data-vccf-church-suite-loader="1"]`)||window.__VCCF_CHURCH_SUITE__)return;
 const s=document.createElement('script');s.src=realSuite+'?v=final1';s.async=false;s.dataset.vccfChurchSuiteLoader='1';document.head.appendChild(s);
}
...
function waitForSuite(){
 ...
 const tick=()=>{if(q('#churchSuiteView')||q('[data-suite-nav]'))return;if(Date.now()-started<10000){ensureChurchLoader();setTimeout(tick,250)}};tick();
}
function bindNav(){
 ...
 nav.addEventListener('click',e=>{
   ...
   const target=coreIds[v]?document.getElementById(coreIds[v]):null;
   if(target){e.preventDefault();e.stopPropagation();activate(v,b)}
 },true);
 qa('.nav button').forEach(b=>{
   const v=b.dataset.view||b.dataset.suiteV2||b.dataset.vccfCoreNav;
   if(v==='church-management')b.dataset.vccfFinalChurchNav='1';
 });
}
function addCss(){
 if(q('#vccf-nav-final-style'))return;
 const s=document.createElement('style');s.id='vccf-nav-final-style';s.textContent=`
 .nav{overflow-y:auto!important;overflow-x:hidden!important;max-height:calc(100vh - 150px)!important;overscroll-behavior:contain!important;scrollbar-width:thin}
 .nav button{position:relative!important;z-index:25!important;pointer-events:auto!important;flex:none!important;touch-action:manipulation!important}
 .nav .nav-label{pointer-events:none!important}
 @media(max-width:700px){.nav{max-height:calc(100dvh - 165px)!important;padding-bottom:18px!important}}
 `;document.head.appendChild(s);
}
function boot(){
 addCss();ensureChurchLoader();ensureChurchFallback();bindNav();waitForSuite();
 const app=q('#app');if(app&&!app.dataset.vccfFinalObserved){app.dataset.vccfFinalObserved='1';new MutationObserver(()=>{if(app.classList.contains('active')){ensureChurchFallback();bindNav()}}).observe(app,{attributes:true,attributeFilter:['class'],childList:true,subtree:true})}
 setTimeout(()=>{ensureChurchFallback();bindNav()},500);setTimeout(()=>{ensureChurchFallback();bindNav()},1500);
}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',boot,{once:true});else boot();
window.addEventListener('vccf-authenticated',boot);window.addEventListener('vccf-app-ready',boot);
})();
