(()=>{
'use strict';
if(window.__VCCF_SAFE_DASHBOARD_VIDEO_V3__)return;
window.__VCCF_SAFE_DASHBOARD_VIDEO_V3__=true;
...
function activeDashboard(){
  ...
  return !!(app?.classList.contains('active')&&dashboard?.classList.contains('active'));
}
async function renderVideo(){
  ...
  const nextSrc=id?`https://www.youtube-nocookie.com/embed/${esc(id)}`:'';
  const currentIframe=panel.querySelector('iframe[data-vccf-video="1"]');
  const currentSrc=currentIframe?.getAttribute('src')||'';
  const currentId=panel.dataset.videoId||'';
...
function boot(){
  ...
  window.addEventListener('vccf-app-ready',()=>schedule(250));
  const app=document.getElementById('app');
  if(app){
    const mo=new MutationObserver(()=>{
      if(activeDashboard())schedule(150);
    });
    mo.observe(app,{attributes:true,subtree:true,attributeFilter:['class']});
    window.__VCCF_SAFE_DASHBOARD_VIDEO_OBSERVER_V3__=mo;
  }
}
