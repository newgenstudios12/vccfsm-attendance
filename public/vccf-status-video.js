(()=>{
'use strict';
if(window.__VCCF_DASHBOARD_LOADER_V22__)return;
window.__VCCF_DASHBOARD_LOADER_V22__=true;
const stamp='202609011735';
...
function showLoginMessage(text,ok=false){const box=document.getElementById('vccfLoginError');if(!box)return;box.style.background=ok?'#ecfdf3':'#fff1f1';box.style.color=ok?'#027a48':'#b42318';box.textContent=text;}

// Single authoritative authentication path. Successful Supabase auth immediately opens the app;
// profile/dashboard loading is deliberately non-blocking.
if(!window.__VCCF_LOGIN_CAPTURE_V3__){
  ...
  document.addEventListener('submit',async event=>{
    ...
    try{
      ...
      // Load the profile only after the UI has transitioned. Profile/RLS issues must not send the user back to login.
      Promise.resolve().then(async()=>{
        try{
          ...
          if(p){
            ...
            if(nameEl)nameEl.textContent=next.name;if(roleEl)roleEl.textContent=next.role+(area?' · '+area:'');if(avatar)avatar.textContent=next.name.charAt(0).toUpperCase();if(accountInfo)accountInfo.textContent=next.name+' · '+next.role;
      ...
      const app=document.getElementById('app');if(app)app.classList.remove('active');
      const login=document.getElementById('login');if(login)login.style.display='grid';
    }finally{
