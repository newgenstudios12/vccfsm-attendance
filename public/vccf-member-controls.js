(() => {
  ...
  function scheduleProfileDedupe(){
    [0,120,300,700,1500,3000].forEach(ms=>setTimeout(dedupeProfileNav,ms));
  }
  ...
  document.addEventListener('DOMContentLoaded',()=>{dedupeProfileNav();scheduleProfileDedupe();boot()});
  window.addEventListener('vccf-app-ready',()=>{dedupeProfileNav();scheduleProfileDedupe();setTimeout(boot,200)});
  document.addEventListener('click',e=>{if(e.target.closest('.nav button,.nav a'))scheduleProfileDedupe()});
  scheduleProfileDedupe();
})();