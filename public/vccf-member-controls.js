(() => {
  ...
  function restoreAttendanceNav(){
    const attendance=document.querySelector('.sidebar .nav button[data-view="attendance"]');
    if(!attendance)return;
    attendance.classList.remove('hidden');
    attendance.hidden=false;
    attendance.removeAttribute('hidden');
    attendance.style.removeProperty('display');
  }
  function watchNav(){
    const sidebar=document.querySelector('.sidebar');if(!sidebar||sidebar.dataset.vccfNavWatch)return;
    sidebar.dataset.vccfNavWatch='1';
    const observer=new MutationObserver(()=>{dedupeProfileNav();restoreAttendanceNav()});
    observer.observe(sidebar,{childList:true,subtree:true,attributes:true,attributeFilter:['class','hidden','style']});
  }
  ...
  function boot(){restoreAttendanceNav();const section=$('members'),toolbar=section?.querySelector('.toolbar');if(!toolbar)return;let box=$('vccfMemberControls');if(!box){box=document.createElement('div');box.id='vccfMemberControls';box.style.cssText='display:flex;flex-wrap:wrap;align-items:center;gap:10px;margin:0 0 16px';toolbar.insertAdjacentElement('afterend',box);box.innerHTML='<span id="vccfMemberCount" style="font-weight:800;color:var(--muted);padding:10px 2px"></span><label style="display:flex;align-items:center;gap:7px;font-weight:800;font-size:.85rem">Sort by: <select id="vccfSortBy" class="search" style="min-width:150px"><option value="name">Name</option><option value="address">Address</option></select></label><label style="display:flex;align-items:center;gap:7px;font-weight:800;font-size:.85rem">Filter by: Address <select id="vccfFilterByAddress" class="search" style="min-width:230px"><option value="">All addresses</option></select></label>';$('vccfSortBy').onchange=e=>{sortMode=e.target.value;render()};$('vccfFilterByAddress').onchange=e=>{addressFilter=e.target.value;apply()}}refresh()}
  document.addEventListener('DOMContentLoaded',()=>{dedupeProfileNav();restoreAttendanceNav();watchNav();boot();setTimeout(()=>{dedupeProfileNav();restoreAttendanceNav();boot()},700)});
  window.addEventListener('vccf-app-ready',()=>setTimeout(()=>{dedupeProfileNav();restoreAttendanceNav();watchNav();boot()},200));
  document.querySelectorAll('.nav button[data-view]').forEach(b=>b.addEventListener('click',()=>setTimeout(()=>{dedupeProfileNav();restoreAttendanceNav();boot()},120)));
})();