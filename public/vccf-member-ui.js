(() => {
  ...
  function memberIdFromRow(row){
    ...
    for(const v of vals){
      const m=(db?.members||[]).find(x=>String(x.id)===v || String(x.memberCode||'')===v);
  ...
  async function saveMemberStatus(id,status){
    ...
    if(!row?.id) throw new Error('Status update returned no updated member.');
    return row;
  }
  ...
  function attendanceNames(){
    try{
      ...
      document.querySelectorAll('#attendanceRows tr,#recentAttendance tr').forEach(row=>{
        const first=row.cells?.[0];if(!first)return;
        const key=first.querySelector('small')?.textContent?.trim()||'';
        const m=byId.get(key)||byCode.get(key);if(!m)return;
        const name=m.name||m.display_name||'?';
        if(first.querySelector('b')?.textContent?.trim()===name)return;
        first.innerHTML=`<div class="member-cell"><span class="member-avatar sm">${esc(name.slice(0,1).toUpperCase())}</span><div><b>${esc(name)}</b><br><small style="color:var(--muted)">${esc(m.memberCode||m.id)}</small></div></div>`;
      });
    }catch(e){console.warn('VCCF attendance repair:',e);}
  }
  ...
  async function repairMembersTable(){
    ...
    [...table.tBodies[0].rows].forEach(row=>{
      ...
      cells.forEach((src,i)=>{
        ...
        } else if(i===6){if(viewButton)td.appendChild(viewButton);}
        else if(i===7){
