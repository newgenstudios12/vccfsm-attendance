(()=>{
'use strict';
if(window.__VCCF_MEMBER_ACCESS_V2__)return;
window.__VCCF_MEMBER_ACCESS_V2__=true;
...
function installLoadDbPatch(){
 if(typeof window.loadDb!=='function'||window.__VCCF_MEMBER_ACCESS_LOADDB_V2__)return;
 window.__VCCF_MEMBER_ACCESS_LOADDB_V2__=true;
 const original=window.loadDb;
 window.loadDb=async function(){await original.apply(this,arguments);cachedAreas=[];await loadAreas();await syncMemberTypes();};
}
function installAreaPatch(){
 if(typeof window.areaMembers!=='function'||window.__VCCF_MEMBER_ACCESS_AREA_PATCH_V2__)return;
 window.__VCCF_MEMBER_ACCESS_AREA_PATCH_V2__=true;
...
function installCheckinPatch(){
 if(typeof window.checkin!=='function'||window.__VCCF_MEMBER_ACCESS_CHECKIN_PATCH_V2__)return;
 window.__VCCF_MEMBER_ACCESS_CHECKIN_PATCH_V2__=true;
...
function splitAddress(raw){
 ...
 if(s.endsWith(suffix)){
  ...
  return {mode:'local',street:parts.join(',').trim(),barangay,other:''};
 }
...
function installMemberModalPatch(){
 if(typeof window.memberModal!=='function'||window.__VCCF_MEMBER_ACCESS_MODAL_PATCH_V2__)return;
 window.__VCCF_MEMBER_ACCESS_MODAL_PATCH_V2__=true;
...
function installMemberRowPatch(){
 if(typeof window.renderMembers!=='function'||window.__VCCF_MEMBER_ACCESS_ROWS_PATCH_V2__)return;
 window.__VCCF_MEMBER_ACCESS_ROWS_PATCH_V2__=true;
