...
function run(){removeDuplicateProfiles();cleanupStraySourceText();loadSpecialEventAccess();maybeLoadAdminAccountManager();loadNavigationCleanup()}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',run,{once:true});else run();
document.addEventListener('click',e=>{if(e.target.closest?.('.nav button[data-view="settings"]')){cleanupStraySourceText();setTimeout(maybeLoadAdminAccountManager,80)}});
const observer=new MutationObserver(()=>{removeDuplicateProfiles();cleanupStraySourceText();maybeLoadAdminAccountManager()});observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['class']});
})();
