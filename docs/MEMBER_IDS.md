...
Area Leaders can edit member records and contact details within their assigned area. Both member detail screens expose the edit action, and the area selector is locked for Leaders. The member photo-only trigger permits authorized leader edits while checking both old and new area IDs; existing RLS still scopes reads and writes. Verified all six assigned Leader accounts, contact RPC, cross-area denial, transfer denial, permanent number protection, Admin/Pastor edits, and ordinary-user photo-only edits in rolled-back database transactions. Both member edit forms were also verified.
